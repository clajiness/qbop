module Service
  ...
  class Counter
    def required_attempts
      db_execute('select required_attempts from counters where id = 1')
    end

    def opnsense_attempt
      db_execute('select opnsense_attempt from counters where id = 1')
    end

    def qbit_attempt
      db_execute('select qbit_attempt from counters where id = 1')
    end

    def opnsense_change
      db_execute('select opnsense_change from counters where id = 1')
    end

    def qbit_change
      db_execute('select qbit_change from counters where id = 1')
    end
